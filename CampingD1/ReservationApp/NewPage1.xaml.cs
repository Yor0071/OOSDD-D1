...
public partial class NewPage1 : ContentPage
{
    ...
    private async void OnReservationButtonClicked(object sender, EventArgs e)
    {
        ...
        await this.ShowPopupAsync(popup); // Dit toont de popup
    }
    ...
    private async void OnCircleTapped(Frame circle)
    {
        selectedCircle = circle;

        // Haal de ID van de cirkel op
        int circleId = circleMap[circle];

        // Navigeren naar ReservationPage en de cirkel-ID doorgeven via BindingContext
        await Navigation.PushAsync(new ReservationPage
        {
            BindingContext = new { CircleId = circleId }
        });
    }
